...
import stylesCss from 'themes/questionnaire/styles.css';
...
window.Vaadin['_vaadintheme_questionnaire_globalCss'] = window.Vaadin['_vaadintheme_questionnaire_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_questionnaire_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_questionnaire_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_questionnaire_globalCss'].push(target);
  }
  if (!document['_vaadintheme_questionnaire_componentCss']) {
    
    document['_vaadintheme_questionnaire_componentCss'] = true;
  }
